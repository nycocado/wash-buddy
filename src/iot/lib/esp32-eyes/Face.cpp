...
u8g2(U8G2_R0, /* reset=*/U8X8_PIN_NONE, /* clock=*/19, /* data= */ 21);

Face::Face(uint16_t screenWidth, uint16_t screenHeight, uint16_t eyeSize)
    : LeftEye(*this), RightEye(*this), Blink(*this), Look(*this),
      Behavior(*this), Expression(*this)
{

    // Unlike almost every other Arduino library (and the I2C address scanner
    // script etc.) u8g2 uses 8-bit I2C address, so we shift the 7-bit address
    // left by one
