...
{
    private:
        // Yes, everything is currently still accessible. Be responsible and
    ...
    public:
        ...
        int frameInterval = 20;     // default value for 50 frames per second
        ...
        int blinkIntervalVariation =
            4; // interval variaton range in full seconds, random number inside
               // of given range will be add to the basic blinkInterval, set to
               // 0 for no variation
        ...
        int idleIntervalVariation =
            3; // interval variaton range in full seconds, random number inside
               // of given range will be add to the basic idleInterval, set to 0
               // for no variation
        ...
        // Startup RoboEyes with defined screen-width, screen-height and max.
        // frames per second
        ...
        // Calculate frame interval based on defined frameRate
        void setFramerate(uint8_t fps) { frameInterval = 1000 / fps; }

        // Set color values
        void setDisplayColors(uint8_t background, uint8_t main)
        {
            BGCOLOR = background; // background and overlays, choose 0 for
                                  // monochrome displays and 0x00 for grayscale
                                  // displays such as SSD1322
            MAINCOLOR = main; // drawings, choose 1 for monochrome displays and
                              // 0x0F for grayscale displays such as SSD1322
                              // (0x0F = maximum brightness)
        }
        ...
        void setPosition(EyePosition position)
        {
            switch (position)
            {
                case EyePosition::N:
        ...
        // Set automated eye blinking, minimal blink interval in full seconds
        // and blink interval variation range in full seconds
        ...
        // Set idle mode - automated eye repositioning, minimal time interval in
        // full seconds and time interval variation range in full seconds
        ...
        // Set curious mode - the respectively outer eye gets larger when
        ...
        void setHFlicker(bool flickerBit, uint8_t Amplitude)
        {
            ...
            hFlickerAmplitude =
                Amplitude; // define amplitude of flickering in pixels
        }
        ...
        {
            return screenHeight -
                   eyeLheightDefault; // using default height here, because
                                      // height will vary when blinking and in
                                      // curious mode
        }
        ...
        void open()
        {
            eyeL_open = 1; // left eye opened - if true, drawEyes() will take
                           // care of opening eyes again
        ...
        void open(bool left, bool right)
        {
            if (left)
            {
                eyeL_open = 1; // left eye opened - if true, drawEyes() will
                               // take care of opening eyes again
            }
            if (right)
            {
                eyeR_open = 1; // right eye opened
        ...
        // Play confused animation - one shot animation of eyes shaking left and
        // right
        ...
        void drawEyes()
        {
            ...
            // Vertical size offset for larger eyes when looking left or right
            // (curious gaze)
            if (curious)
            {
                if (eyeLxNext <= 10)
            ...
            eyeLy +=
                ...
                ); // vertical centering of eye when closing
            ...
            if (autoblinker)
            {
                if (millis() >= blinktimer)
                {
                    ...
                    blinktimer = millis() + (blinkInterval * 1000) +
                                 (random(blinkIntervalVariation) * 1000
                                 ); // calculate next time for blinking
                }
            }
            ...
            // Confused - eyes shaking left and right for the duration defined
            // by confusedAnimationDuration (default = 500ms)
            if (confused)
            {
                if (confusedToggle)
                {
            ...
            if (idle)
            {
                if (millis() >= idleAnimationTimer)
                {
                    ...
                    idleAnimationTimer =
                        ...
                        (random(idleIntervalVariation) * 1000
                        ); // calculate next time for eyes repositioning
                }
            }
            ...
            // display->clearDisplay(); // start with a blank screen (COMMENTED
            // OUT FOR LAYERED DRAWING)
            ...
            eyelidsTiredHeight =
                (eyelidsTiredHeight + eyelidsTiredHeightNext) / 2;
            if (!cyclops)
            {
                display->fillTriangle(
                    eyeLx,
                    eyeLy - 1,
                    eyeLx + eyeLwidthCurrent,
                    eyeLy - 1,
                    eyeLx,
                    eyeLy + eyelidsTiredHeight - 1,
                    BGCOLOR
                ); // left eye
                display->fillTriangle(
                    eyeRx,
                    eyeRy - 1,
                    eyeRx + eyeRwidthCurrent,
                    eyeRy - 1,
                    eyeRx + eyeRwidthCurrent,
            ...
            eyelidsAngryHeight =
                (eyelidsAngryHeight + eyelidsAngryHeightNext) / 2;
            if (!cyclops)
            {
                display->fillTriangle(
                    eyeLx,
            ...
            if (sweat)
            {
                ...
                else
                {
                    ...
                    sweat1Width = 1;
                    sweat1Height = 2;
                } // if max vertical position is reached: reset all values for
                  // next drop
                ...
                sweat1XPos =
                    sweat1XPosInitial -
                    (sweat1Width / 2); // keep the growing shape centered to
                                       // initial x position
                display->fillRoundRect(
                    sweat1XPos,
                    sweat1YPos,
                ...
                sweat2XPos =
                    sweat2XPosInitial -
                    (sweat2Width / 2); // keep the growing shape centered to
                                       // initial x position
                display->fillRoundRect(
                    sweat2XPos,
                    sweat2YPos,
                ...
                sweat3XPos =
                    sweat3XPosInitial -
                    (sweat3Width / 2); // keep the growing shape centered to
                                       // initial x position
                display->fillRoundRect(
                    sweat3XPos,
                    sweat3YPos,
            ...
            // display->display(); // show drawings on display (COMMENTED OUT
            // FOR LAYERED DRAWING)
