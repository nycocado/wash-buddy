...
/**
 * Contains all functions to draw eye based on supplied (expression-based)
 * config
 */
class EyeDrawer
{
    public:
        ...
        {
            // Amount by which corners will be shifted up/down based on
            // requested "slope"
            int32_t delta_y_top = config->Height * config->Slope_Top / 2.0;
            int32_t delta_y_bottom =
                config->Height * config->Slope_Bottom / 2.0;
            // Full extent of the eye, after accounting for slope added at top
            // and bottom
            auto totalHeight = config->Height + delta_y_top - delta_y_bottom;
            // If the requested top/bottom radius would exceed the height of the
            // eye, adjust them downwards
            ...
            // Calculate _inside_ corners of eye (TL, TR, BL, and BR) before any
            // slope or rounded corners are applied
            ...
            EyeDrawer::FillRectangle(
                TRc_x, TRc_y, BRc_x + config->Radius_Bottom, BRc_y, 1
            ); // Right
            EyeDrawer::FillRectangle(
                TLc_x - config->Radius_Top, TLc_y, BLc_x, BLc_y, 1
            ); // Left
            EyeDrawer::FillRectangle(
                TLc_x, TLc_y - config->Radius_Top, TRc_x, TRc_y, 1
            ); // Top
            EyeDrawer::FillRectangle(
                BLc_x, BLc_y, BRc_x, BRc_y + config->Radius_Bottom, 1
            ); // Bottom
            ...
            if (config->Slope_Top > 0)
            {
                EyeDrawer::FillRectangularTriangle(
                    TLc_x,
                    TLc_y - config->Radius_Top,
                    TRc_x,
                    TRc_y - config->Radius_Top,
                    0
                );
                EyeDrawer::FillRectangularTriangle(
                    TRc_x,
            ...
            // Draw corners (which extend "outwards" towards corner of screen
            // from supplied coordinate values)
            if (config->Radius_Top > 0)
            {
                EyeDrawer::FillEllipseCorner(
                    T_L, TLc_x, TLc_y, config->Radius_Top, config->Radius_Top, 1
                );
                EyeDrawer::FillEllipseCorner(
                    T_R, TRc_x, TRc_y, config->Radius_Top, config->Radius_Top, 1
                );
            }
