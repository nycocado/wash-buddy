...
eEmotions FaceBehavior::GetRandomEmotion()
{
    ...
    // Now pick a random number that lies somewhere in the range of total
    // weights
    float rand = random(0, 1000 * sum_of_weight) / 1000.0;
    // Loop over emotions and select the one whose probabity distribution
    // contains the value in which the random number lies
    float acc = 0;
    for (int emotion = 0; emotion < eEmotions::EMOTIONS_COUNT; emotion++)
    {
